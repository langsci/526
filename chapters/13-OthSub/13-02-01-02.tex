...
lll}
\lsptoprule
%\tbx{13-11}	&	PMP	&	Kambera	&	gloss	\\	\midrule
%\endfirsthead
\tbx{13-11}	&	PMP	&	Kambera	&	gloss	\\	\midrule
%\endhead
...
The evidence from Kambera is sufficient to affirm Blust’s assessment
...
Kambera example described above,
even when reduction is involved.
For example, data from the \tsc{Seram-Tanimbar-Bomberai} language
Kei Kecil shows that a single language may have more than one
...
Buru also shows that a single sound correspondence cannot be
assumed for similar medial *NC sequences in the same language.
Buru has no root internal medial CC clusters,
so it reduces medial clusters,
even in loans, as in Malay \it{ganti} > Buru \it{gati} ‘replace,
change’; Malay \it{tuŋgu} ‘wait,
keep watch, guard’ > Buru \it{tugu} ‘guard (against intrusion), keep watch, wait’.
...
The regular sound correspondence for Buru is PMP *d > \it{r},
...
\lsptoprule
%\tbx{13-12}	&	PMP gloss	&	PMP	&	Buru	&		\\	\midrule
%\endfirsthead
\tbx{13-12}	&	PMP gloss	&	PMP	&	Buru	&		\\	\midrule
%\endhead
...
Like Kambera, the Buru data also show that the *NC clusters must
...
And like Kei Kecil,
the Buru data also show that a single language may have more
...
Ngaju \it{dindiŋ}, Malagasy \it{rindrina},
Iban \it{dindiŋ}, Malay \it{dindiŋ},
Sasak \it{dindiŋ} (data from \citealp{BlustTrussel2020}).
Furthermore, within the target region it appears the nasal may
occasionally be retained unassimilated,
as in the Southwest Tarangan
and Dobel forms for ‘dark’ in \trf{13-08},
...
*dəm.\footnote{
		...
		material, quite different from the Southwest Tarangan
		and Dobel forms.}
The collective data together show a variety
of patterns, including coalescence (→ C\sub{3}) in the Kaitetu
and Saparua forms in what happens to the medial cluster,
and loss or haplology in Tii and Galolen.
...
\tsc{BL}	&	Kambera	&	‹mari\tbr{ng}u›	&	{\hr}ru\tbr{nd}uŋ-u	&	{\hr}ndu\tbr{r}u	&	\\
\tsc{BL}	&	Dhao	&	{\hr}mari\tbr{ŋ}i	&		&	{\hr}ɗo\tbr{r}o	&	\\
\tsc{TB}	&	Tii	&		&		&	{\hr}ruu	&	\\
\tsc{TB}	&	Helong	&	{\hr}b-li\tbr{ŋ}in	&	{\hr}de\tbr{d}eŋ	&	{\hr}lo\tbr{t}o	&	\\
\tsc{TB}	&	Tetun	&	{\hr}ma-li\tbr{r}in	&		&	{\hr}taru\tbr{t}u	&	\\
\tsc{TB}	&	Galolen	&	{\hr}na-diin	&		&		&	\\
\tsc{TB}	&	Rasua	&	{\hr}na-ri\tbr{ɲ}i	&		&		&	\\
\tsc{TB}	&	Wetan	&	{\hr}ri\tbr{nn}i	&		&		&	\\
\tsc{TB}	&	Selaru	&	{\hr}mrin	&		&	{\hr}kdu\tbr{d}u,	&	{\hr}‘rumble’\\
	&		&		&		&	{\hr}kru\tbr{t}u	&	{\hr}‘thunder’\\
\tsc{Aru}	&	SW Tarangan	&		&	{\hr}de\tbr{m}ur	&	{\hr}du\tbr{d}im-na	&	\\
\tsc{Aru}	&	Dobel	&		&	{\hr}de\tbr{m}ur	&	{\hr}fadu\tbr{d}um,	&	{\hr}‘thunder’\\
	&		&		&		&	{\hr}du\tbr{d}um	&	{\hr}‘sound’\\
\tsc{Aru}	&	Kola	&		&	{\hr}do\tbr{d}im	&	{\hr}ahdu\tbr{d}um	&	\\
STB	&	Yamdena	&	{\hr}ndi\tbr{r}in	&	{\hr}nro\tbr{nr}am	&	{\hr}nru\tbr{nr}um	&	\\
STB	&	Fordata	&	{\hr}ri\tbr{d}in	&	{\hr}de\tbr{d}èn	&	{\hr}du\tbr{d}ur	&	\\
STB	&	Kei	&	{\hr}ri\tbr{d}in, ri\tbr{n}in	&	{\hr}de\tbr{d}an	&	{\hr}do\tbr{d}	&	\\
STB	&	Irarutu	&	\mc{2}{l}{{\hr}ri\tbr{nd}ən, rí\tbr{d}ənə}		&		&	\\
STB	&	Kowiai	&	{\hr}ri\tbr{d}in	&		&	{\hr}du\tbr{d}un	&	\\
STB	&	Geser	&	{\hr}mari\tbr{h}i	&		&	{\hr}du\tbr{d}un	&	\\
Banda	&	Banda	&	{\hr}ri\tbr{nd}in	&		&		&	\\
\tsc{AS}	&	Alune	&	{\hr}ndi\tbr{r}i	&		&		&	\\
\tsc{AS}	&	Wakasihu	&	{\hr}li\tbr{r}inu	&		&		&	\\
\tsc{AS}	&	Kaitetu	&	\mc{2}{l}{{\hr}pa-ri\tbr{k}i (reg. *nd > \it{k})}			&	\\
\tsc{AS}	&	Saparua	&	\mc{2}{l}{{\hr}pi-ri\tbr{k}i (reg. *nd > \it{k})}			&	\\
\tsc{AS}	&	Kayeli	&	{\hr}namni\tbr{r}i	&		&		&	\\
\tsc{SB}	&	Buru	&	{\hr}eb-ri\tbr{d}i	&	{\hr}re\tbr{d}e(n)	&		&	\\
		\lspbottomrule
	\end{tabular}
		\begin{tablenotes}\tnsize
			%\item[†]	{Kambera ‹tau ringu› ‘people from cool areas,
