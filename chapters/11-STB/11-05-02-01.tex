...
The \tsc{Nuclear East Bomberai} node contains Arguni, Bedoanas, and Erokwanas.
Of all the \tsc{Tanimbar-Bomberai} languages,
the inclusion of these languages is the least certain.
...
\begin{enumerate}
	\item	*ijuŋ could be linked with Arguni \it{sigyó},
				Bedoanas, Erokwanas \it{suʔu} ‘nose’ with irregular loss of the
				...
				vowel is [e] in Arguni
				and [a] in Bedoanas and Erokwanas.
				We would also need to posit irregular *u > \it{i} in Arguni to
				derive \it{sigyó} from *ijuŋ.
				Thus, these words are unlikely to be connected.
				If they are, they show *j > \it{s}.
	\item	*pija > Arguni \it{fís} ‘how much?’ could be a loan from
				Biak, Kowiai, or Geser \it{fis}, or the final consonant may be a suffix
				(compare Onin and Sekar \it{firas}).
				If this is not a loan,
				it shows *j > \it{s} or *j > {\0}.
	\item	*qapəju > **ma-pəju > Arguni \it{mari} is mostly unproblematic.
	\item	*pajay > Arguni \it{pwásen},
				Bedoanas, Erokwanas \it{pása} ‘rice’ has irregular retention
				of *p = \it{p} where we expect *p > \it{f}.
				It is thus a loan,
				as it is in many other languages of the region.
	\item	*waRəj > Arguni,
				Bedoanas, Erokwanas \it{wárir} ‘rope’ is unproblematic.
	\item	*ŋajan > Arguni,
				Erokwanas \it{gadin}, Bedoanas \it{gadiŋ} ‘name’ is unproblematic.
	\item	*huaji > **ta-waji > Arguni \it{twari} ‘friend’ is unproblematic.
\end{enumerate}

Of all these words,
only *qapəju > **ma-pəju > Arguni \it{mari} ‘bitter’ has *j >
...
and \tsc{Irarutu-Kuri} (as discussed above),
and the evidence for including \tsc{Irarutu-Kuri} within \tsc{Tanimbar-Bomberai}
is relatively good (see \srf{11-05-02-02}).

The evidence for placing Arguni,
Bedoanas, and Erokwanas within a single node comes mainly from
certain lexically specific changes in the vowels.
There are a handful of instances of final *a > \it{i},
two examples of *ə > \it{o},
and at least one example of final *i > \it{u}.
...
Arguni	&	{\hr}gád\tbr{i}n	&	{\hr}púr\tbr{i}n	&	{\hr}rár\tbr{i}n	&	{\hr}mb\tbr{o}	&	{\hr}pw\tbr{ó}res	&	{\hr}ár\tbr{u}t, ur\tbr{u}t	&	{\hr}táòr\\
Bedoanas	&	{\hr}gad\tbr{i}ŋ	&	{\hr}úr\tbr{i}n	&	{\hr}rár\tbr{i}n	&	{\hr}mb\tbr{o}	&	{\hr}\tbr{ó}ras	&	{\hr}ár\tbr{u}t	&	{\hr}taur\\
Erokw.	&	{\hr}gad\tbr{i}n	&	{\hr}púr\tbr{i}n	&	{\hr}rór\tbr{i}n	&	{\hr}b\tbr{o}	&	{\hr}p\tbr{ó}ras	&	{\hr}ar\tbr{u}t	&	{\hr}tour\\
Irarutu	&	\cg{(no)}	&	\cg{(seba)}	&	{\hr}rarn	&	\cg{(fne)}	&	{\hr}eʤe	&	{\hr}rit	&	{\hr}tor\\
Kuri	&	\cg{(inɔi)}	&	\cg{(sarabɛ)}	&	{\hr}rarn	&	\cg{(bəne)}	&	{\hr}bɔyɛ	&	{\hr}riːtə̆	&	{\hr}tɔr\\
		\lspbottomrule
	\end{tabular}
...
One example is apparent CV > VC metathesis in *təlu > Arguni
\it{táòr}, Bedoanas \it{taur}, and Erokwanas \it{tour} ‘three’.
...
Three examples, with regular **g > \it{ʔ} in Bedoanas
and Erokwanas are: Arguni \it{sigyó},
Bedoanas, Erokwanas \it{suʔu} ‘nose’, and Arguni \it{sigye};
Bedoanas, Erokwanas \it{siʔi} ‘stomach’, as well as Arguni \it{rigígit},
Bedoanas, Erokwanas \it{riríʔit} ‘white’.
