\sscp{Austronesian incursion/migration(s): \tsc{Central Timor}}{15-06-07}
It is not yet clear where Austronesian-speaking peoples came
from to start what eventually developed into the \tsc{Central
Timor} subgroup (see \crf{ch:CT}).
\tsc{Central Timor} languages are surrounded by \tsc{Timor-Babar} languages.
...
There are two logical possibilities:\footnote{
		...
		and drawing the problem of the position of \tsc{Central Timor}
		to our attention.}

\begin{enumerate}
	\item	\tsc{Central Timor} is the remnant of an Austronesian group
				whose extent was curtailed by the later arrival of \tsc{Timor-Babar} languages.
	\item	\tsc{Central Timor} contains descendants of a language
...
two scenarios.\footnote{
		...
		cluster and \tsc{Central Timor} languages which may suggests that \tsc{Central
		Timor} once extended further west into areas where Tetun and/or
		\tsc{Meto} is now spoken,
		though it could also suggest that \tsc{Meto} once extended further
		east into areas where Tetun is now spoken.}
Another factor that
needs to be considered is the connections/relationships between
\tsc{Central Timor}, \tsc{Timor-Babar}, and the neighbouring non-\tsc{An} TAP languages.
More investigation into these connections may by help understand
the pre-history of the \tsc{Central Timor} languages.

\citet{SchapperWellfelt2018} discuss contact between the non-\tsc{An}
languages of Alor and those of Timor,
including \tsc{Central Timor} languages.
They discuss songs sung by some groups in Alor which are in a
language unknown to the groups singing but which originate “[{\ldots}]
from Timor and in particular the Tokodede ethno-linguistic group [{\ldots}]
The songs were recognized [by Tokodede people] although the Tokodede
...
the \tsc{Alor-Pantar} languages had “[{\ldots}] contact with
...
\begin{quote}
		...
		Alor-Timor relations and the prestige group from which influence flowed.
		The regions with most significant contact were the Tokodede region
		(Likisa-Maubara) on Timor and eastern Alor, an unsurprising result given the proximity
		of the two islands.
...
Our knowledge of the \tsc{Central Timor} languages is extremely limited.
For example, Welaun was given an ISO code [wlh] only in 2020.
Similarly, \citet[75--93]{Fogaca2017} shows that the Mambae [mgm]
...
found between Northwest Mambae and Tokodede.
More data and careful bottom-up reconstruction may help us better
understand the pre-history of the \tsc{Central Timor} languages.