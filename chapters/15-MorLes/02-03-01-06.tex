...
relating to daily relationships (such as English \it{his heart
...
\midrule
&	English			&	{\hr}heart					&	/hɑːt/			&	1) heart; 2) emotions		\\
...
&	Sanskrit		&	{\hr}\Sanskrit{हृदय}	&	/ɦr̩d̪aya/		&	1) heart; 2) emotions		\\
...
\lsptoprule
\tbx{02-37}&	Tagalog	&	atay	&	1) liver; 2) emotions		\\
&	Malay	&	hati	&	1) liver; 2) emotions		\\
&	Sunda	&	hate	&	1) liver; 2) emotions		\\
&	Makasar	&	ate	&	1) liver; 2) emotions		\\
&	Wolio	&	ate	&	1) liver; 2) emotions		\\
&	Sasak	&	ate	&	1) liver; 2) emotions		\\
\lspbottomrule
\end{tabular}\mbox{}\\}

Some Austronesian languages east of the Blust line (mostly \tsc{Bima-Lembata}
languages) also maintain this figurative sense of ‘liver’.

{\wg\tblr{>{\hspace{-\tabcolsep}}l<{\hspace{-\tabcolsep}}>{\hspace{-\tabcolsep}\enspace}lll}
\lsptoprule
\tbx{02-38}	&	Bima	&	ade	&	1) liver; 2) emotions		\\
	&	Ngadha	&	ate	&	1) liver; 2) emotions		\\
	&	Sika	&	ʔβate	&	1) liver; 2) emotions		\\
	&	Kambera	&	eti	&	1) liver; 2) emotions		\\
\lspbottomrule
\end{tabular}\mbox{}\\}
...
{\wg\tblr{>{\hspace{-\tabcolsep}}l<{\hspace{-\tabcolsep}}>{\hspace{-\tabcolsep}\enspace}llll}
\lsptoprule
\tbx{02-39}	&	Dhao\footnotemark[20]
	&	ɗara	&	1) inside, 2) emotions	&	\tsc{Bima-Lembata}		\\
%	&	Havu\footnote{}	&	ɗara	&	1) inside, 2) emotions	&	\tsc{Bima-Lembata}		\\
	&	Dela	&	rala-n	&	1) inside, 2) emotions	&	\tsc{Timor-Babar}		\\
	&	Tii	&	dale-n	&	1) inside, 2) emotions	&	\tsc{Timor-Babar}		\\
	&	Helong	&	dale-n	&	1) inside, 2) emotions	&	\tsc{Timor-Babar}		\\
	&	Tetun\footnotemark[21]
	&	lara-n	&	1) inside, 2) emotions
	&	\tsc{Timor-Babar}		\\
	&	Luang	&	ralma ni	&	1) inside, 2) emotions	&	\tsc{Timor-Babar}		\\
	&	S. Mambae	&	hua-lala	&	‘fruit insides/heart insides’	&	\tsc{Central Timor}		\\
	&	Kemak	&	lara-n	&	1) inside, 2) emotions	&	\tsc{Central Timor}		\\
	&	Buru	&	lale-n	&	1) inside, 2) emotions	&	\tsc{Sula-Buru}		\\
	&	Asilulu	&	lale-	&	1) inside, 2) emotions	&	\tsc{Ambon-Seram}		\\
	&	Yamdena	&	dalamy	&	1) inside, 2) emotions	&	\tsc{STB}		\\
\lspbottomrule
\end{tabular}\mbox{}\\}

\footnotetext[20]{
		While Dhao consistently uses \it{ɗara} for ‘seat of emotions’,
		closely related Havu is split, using both \it{ade} ‘liver’
		and \it{ɗara} ‘insides’ for ‘seat of emotions’,
		with \it{ade} being the more frequent.
		So Havu has \it{adu ade} ‘resistant (lit: hard liver)’,
		...
		So while Dhao has shifted to ‘insides’,
		its nearest linguistic relative straddles the fence,
		but remains mostly in the ‘liver’ camp.}\addtocounter{footnote}{1}
\footnotetext[21]{Grimes has worked extensively with both Tetun Dili [tdt]
		and Tetun [tet], and it is clear that the gloss in
		\citet{BlustTrussel2020} for Tetun Dili \it{laran} glossed merely as
		...
		The gloss for Havu \it{ɗara} in \citet{BlustTrussel2020} as ‘in,
...
%\end{envfn}
%\begin{envfn}
%\footnotetext{Grimes has worked extensively with both Tetun Dili [tdt]
		%and Tetun [tet], and it is clear that the gloss in
		%\citet{BlustTrussel2020} for Tetun Dili \it{laran} glossed merely as
		%‘in, inside’ while not wrong,
		%is also incomplete, as reading newspapers or listening to love
...
{\wg\tblr{>{\hspace{-\tabcolsep}}l<{\hspace{-\tabcolsep}}>{\hspace{-\tabcolsep}\enspace}llll}
\lsptoprule
\tbx{02-40}	&	Teiwa	&	gom	&	1) inside; 2) heart	&	TAP		\\
	&	Pura	&	omi	&	1) inside; 2) place of belief	&	TAP		\\
	&	Abolo	&	ʊm	&	1) inside; 2) emotions	&	TAP		\\
	&	Wersing	&	mira	&	1) inside; 2) emotions	&	TAP		\\
\lspbottomrule
\end{tabular}\mbox{}\\}
