...
Unusual features in the Batak languages of Sumatra are sometimes
...
as in Malay \it{rumah=ku} or \it{rumah saya} ‘house \tsc{1sg}’ = ‘my house’.
Body parts tend to be possessed in the same way as other nouns,
as with Malay \it{taŋan=ku} or \it{taŋan saya} ‘hand \tsc{1sg}’
...
and the \tsc{Timor-Alor-Pantar} languages is verb-final SOV, with pre-posed possessors.
...
such as numbers or colours.\footnote{
		...
		case in a variety of Fataluku (a Papuan TAP language spoken on
...
But preposed possessors are also not found in much of the \tsc{Bima-Lembata} subgroup (see \frf{02-16}).
...
As areal features resulting from contact they are not useful
...
Yet Papuan-sourced {\#}muku ‘banana’ made it as far west as Manggarai
in western Flores (\frf{02-20}).
Dhao and Havu use ‘leaf of head’ to talk about ‘hair’,
...
The transition in languages from Java to Bali to Lombok to Sumbawa
to western Flores is relatively gradual,
with a boundary in the middle of Sumbawa separating the western languages from \tsc{Bima-Lembata}.

\item	Near the northern parts of our target region,
just west of the Blust line,
in Banggai (Sulawesi) and Taliabu, preposed possessors are also found.
...
there is a pocket of languages in Central Flores that appear
to have very little morphology (Elias 2018, 2020; McWhorter 2019).
These form a node within our \tsc{Bima-Lembata} subgroup.
...
there is a subgroup in \tsc{Central Timor} that has a different
...
\item	Within neighbouring subgroups or nodes,
			...
			For example, \tsc{Ambon-Seram} languages have subject indexing
			...
			Do the differences in \tsc{Ambon-Seram} languages reflect substrate
...
\item	When we reach Aru or the Bomberai peninsula of west Papua,
			...
			readers who are familiar with Malay,
			Balinese, Tagalog, or Cebuano should skim the Dobel examples provided in \citet{Hughes2000}.
...
between Island Southeast Asian (Austronesian) languages,
and Melanesian (Papuan) languages.
So at the western edge of the zone of transition we find the
languages in western Flores
and Sumba-Havu show only a few features that are associated with Papuan languages.
But there are some.
