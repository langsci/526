...
Mangole, and Sanana have preposed genitive proclitics/prefixes
while other languages in the region have postposed enclitics/suffixes.
In \tsc{Taliabu} and Sanana the possessed noun takes a prefix
...
Soboyo	&	{\hr}n-ade	&	{\hr}n-oyo-in	&	{\hr}n-uhu	&	{\hr}n-ama	&	{\hr}m-fulu-in	&	{\hr}uha	&	\cg{(talaɲ)}\\
Taliabu	&	{\hr}n-ade	&	{\hr}n-oyu-ɲ	&	{\hr}n-uhu	&		&	{\hr}fulu-ɲ	&	{\hr}n-uha-c	&	\cg{(talaŋ)}\\
Kadai	&	{\hr}n-ade	&	{\hr}n-oyu	&		&	{\hr}n-ama	&	{\hr}m-fulu	&	{\hr}n-uha	&	\cg{(n-tala)}\\ \midrule
Mangole	&		&	{\hr}n-peu	&	{\hr}opu	&		&	{\hr}n-foro foo	&		&	{\hr}loi\\
Sanana	&	{\hr}n-aya	&	{\hr}m-peu	&	{\hr}opu	&	{\hr}n-ama	&	{\hr}foa	&	{\hr}ua	&	{\hr}hoi\\
Lisela	&	{\hr}aa-n	&	{\hr}peu-n	&	{\hr}upu-n	&	{\hr}ŋ-ama\su{‡}	&	{\hr}ɸulu-n	&		&	{\hr}rohi-n\\
Buru	&	{\hr}aa-n	&	{\hr}peu-n	&	{\hr}opo-n	&	{\hr}ama-n	&	{\hr}folo-n	&	{\hr}uha-n	&	{\hr}rohi-n\\ \midrule
Ambelau	&	{\hr}ala-mu\su{†}	&	{\hr}feo-ni	&	{\hr}opo-ni	&		&	{\hr}bolu-e	&		&	\cg{(kukui-ni)}\\
		\lspbottomrule
	\end{tabular}
		\begin{tablenotes}\tnsize
			\item[†] {Ambelau \it{ala-mu} is `chin/jaw-\tsc{2sg}' = `your chin/jaw'.}
			\item[‡] {Lisela \it{ŋ-ama} is borrowed from Sanana.}
			%\item[Δ]
			%\item[‖]
			%\item[¶]
		\end{tablenotes}
...
\citet[68]{Mead2003b} notes that Banggai also has the preposed
possessor widely associated with eastern Indonesia,
such as \it{mian lima-no} ‘person hand-\tsc{3sg}’
and \it{koŋgu bonua} ‘my house’.
Other \tsc{Saluan-Banggai} languages have post-posed possessors
