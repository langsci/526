...
\tsc{Ambon-Seram} from \tsc{Sula-Buru} and convey the sense to anyone working with languages in both
subgroups that the two are very different.
\tsc{Ambon-Seram} languages have Actor-Undergoer inflection or
...
and \tsc{Ambon-Seram} languages is built around mechanisms that
...
While modern Buru no longer functions as a Split-S system,
...
and the notion of grammatical Subject in central Maluku languages.\footnote{
		...
		Split-S languages of various types are also found in \tsc{Bima-Lembata},
		\tsc{Timor-Babar}, \tsc{Aru}, \tsc{Seram-Tanimbar-Bomberai},
		and SHWNG languages, as well as west of the Blust line.}

The Split-S variations in \tsc{Ambon-Seram} languages on Seram
and Ambon-Lease include some with a pronominal marking in which
...
Semantic alignment of this sort found in \tsc{Ambon-Seram} languages
...
Yalahatan active transitive clauses have the subject-Actor \it{preceding}
...
\setcounter{xnumi}{4}
	\ex{\gll	si=hita ulu-si\\
						...
						\jambox*{(Yalahatan: active transitive)}
...
\begin{exe}
	\ex{\gll	{\ldots} lau mata=\tbf{ʔu}\\
						{} until die=\tsc{1sg.u}\\}
						\jambox*{(Yalahatan: non-active intransitive)}
			\glt	‘{\ldots} until I die.’
						\label{ex:07-07}
\end{exe}

Some languages in the central Maluku region,
such as Sou Amana Teru \citep{Musgrave2010}
and Yalahatan have verbs of motion,
...
Compare Buru examples \qf{07-10}--\qf{07-11} with Yalahatan example \qf{07-09}.
...
\begin{exe}
	\ex{\gll	\tbf{yako} sei \tbf{kono} haik fi di nani fina emsawan rua naa.\\
			...
			\glt	‘I am fed up with these two daughters-in-law of ours.’ \hfill{(Buru: experiencer)}
						\label{ex:07-10}
	\ex{\gll	\tbf{yako} la ep-toke emhewak \tbf{kono}, {\ldots}\\
						\tsc{1sg} \tsc{irr} \tsc{caus}-show self \tsc{1sg.u}\\}
						\jambox*{(Buru: reflexive)}
			\glt	‘I want to teach myself, [how to read and write Buru].'
						\label{ex:07-11}
\end{exe}
...
The data in \qf{07-12} are from Western Pantar (a Papuan Timor-Alor-Pantar
language).

\begin{exe}
	\ex{\gll	naŋ na-lama ta.\\
						\tsc{1sg} \tsc{1sg.u}-go \tsc{ipfv}\\}
						\jambox*{(Western Pantar: motion; \citealp[105]{Holton2010a})}
			\glt	‘I'm going.’
						\label{ex:07-12}
\end{exe}
...
in Klon \citep[50--51, 81--84]{Banamtuan2021} as in examples \qf{07-13}--\qf{07-14},
a Timor-Alor-Pantar language of Alor.

\begin{exe}
	\ex{\gll	naan na agai.\\
						\tsc{1sg} \tsc{1sg.u} go\\}
						\jambox*{(Klon: motion; \citealp[84]{Banamtuan2021})}
			\glt	‘I'm going now.’
						\label{ex:07-13}
	\ex{\gll	ga a moop.\\
						\tsc{3sg} \tsc{3sg.u} sleep\\}
						\jambox*{(Klon: posture; \citealp[51]{Banamtuan2021})}
			\glt	‘He slept.’
						\label{ex:07-14}
\end{exe}

In the morphosyntax, there are three related issues that distinguish
\tsc{Ambon-Seram} languages from \tsc{Sula-Buru} languages.

\begin{itemize}
	\item	\tsc{Ambon-Seram} languages tend to inflect verbs for person
	...
	\item	\tsc{Ambon-Seram} languages can have several different inflectional
				...
				(see \trf{07-16}).
	\item	\tsc{Ambon-Seram} languages distinguish between human
...
\begin{itemize}
	\item	\tsc{Ambon-Seram} languages distinguish between alienable
...
As mentioned in \srf{01-07},
it is important for comparative purposes to note that the inflectional
paradigms for verbs in \tsc{Ambon-Seram} languages are only partially
...
As mentioned above, \tsc{Ambon-Seram} languages tend to inflect
...
several verbs for a number of \tsc{Ambon-Seram} languages.
In Asilulu \citep[xxix--xxx]{Collins2003a} these are restricted to Active verbs.
\citet[36--42]{Bolton1990} describes the verb inflections for South Nuaulu.
\citet[86--91]{Laidig1992} describes these for Larike,
with some morphophonemic alternations of the initial {\#}C of
the verb root illustrated in \trf{07-16}.

\begin{table}\tcp{Subject indexing in Larike showing morphophonemics}{07-16}
...
\tsc{3pl.nhum}	&	iri-	&	-\tbb{p}ese	&	-\tbb{k}iʔi	&	-\tbr{r}aʔa	&	\hr-\tbr{r}iwa	\\	
	...
	\end{tabular}
		\begin{tablenotes}\tnsize
			\item[†] {Compare Buru \it{tewa} ‘know, understand’,
								Asilulu \it{tewa} ‘know’, Alune \it{tekwa} ‘know’.}
		\end{tablenotes}
	\end{threeparttable}
\end{table}

\newpage
Apart from \tsc{Ambon-Seram} languages,
similar morphophonemic changes in the initial consonants also
occur in the \tsc{Eastern Islands} language Geser-Gorom,
...
in Banda, as illustrated in \trf{07-17}.
In Banda these alternations often involve a nasal or prenasalised
consonant, though other consonants are also involved.

\begin{table}\tcp{Subject indexing in Banda showing morphophonemics}{07-17}
	\begin{threeparttable}\st{6pt}
...
In the morphosyntax, \tsc{Ambon-Seram} languages also distinguish
...
\citet[28]{Collins1983} also provides additional data for other \tsc{Ambon-Seram} languages.
Other languages of central Maluku do not appear to make such
a human/non-human distinction.\footnote{
		\citet{Bloyd2020} gives
		contradictory information on this for the Sula (Sanana) language.
...
Thus, the human/non-human distinction sets \tsc{Ambon-Seram}
apart from neighbouring subgroups.
Note, however, that languages of Aru also have a human/non-human
...
linguistic Wallacea.\footnote{
		...
		descriptions of \tsc{Ambon-Seram}
		languages use the terms human/non-human.} 

\begin{table}
	\centering\tcp{Human/non-human marking in \tsc{Ambon-Seram} languages}{07-18}
