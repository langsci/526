\largerpage
\ssscp{\tsc{\ili{Irarutu}-Kuri}}{11-05-02-02}
Of all the \tsc{East Bomberai} languages,
the position of \ili{Irarutu}
and closely related \ili{Kuri} (a.k.a.
\ili{Nabi}) has remained the most unclear.
While \citet{Blust1993} linked \ili{Arguni} with the other Austronesian
languages of the Bomberai peninsula,
he was unable to reach any firm position for \ili{Irarutu} stating
...
assessed \ili{Irarutu} as probably having “some kind of shared history
...
\citet[242f]{Jackson2014} presents a discussion of \ili{Irarutu} historical
phonology and after considering the various hypotheses concludes that \ili{Irarutu}
is probably a primary branch of CEMP which has undergone contact with SHWNG languages.
He notes that classifying \ili{Irarutu} as SHWNG would probably require
positing borrowing from an unknown non-SHWNG Austronesian source.\footnote{
		\citet{SchapperZobel2024b} link \ili{Irarutu} with \ili{Kowiai} and
		thus within our \tsc{Eastern Islands}.
		See footnote \ref{fn:SchapperZobel2024}.}

Part of the difficulty with classifying \ili{Irarutu} is due to the
heavily Papuanised nature of the language with few clear Austronesian inheritances.
In addition, words in \ili{Irarutu} are usually reduced to monosyllables
...
Whether \ili{Irarutu} and \ili{Kuri} should be classified as SHWNG can be
...
Each potential reflex of PMP *j is discussed below.\footnote{
		In addition to these six words *waRəj > \ili{Irarutu} \it{wara} ‘rope’
		has loss of final *j.}
...
\begin{enumerate}
	\item	*pajay > \it{fas} [ɸasə] ‘rice’ is probably a loan.
				...
				\ili{As} discussed in relation to \trf{11-04} on \prf{tab:11-04},
				all other Austronesian Bomberai languages (except \ili{Uruangnirin})
				...
				*p > \it{f}, providing further evidence that this word has spread by contact.
				...
				Potential donors include \ili{Geser} \it{fasa} ‘rice’ or neighbouring \ili{Wamesa} \it{pas}.
				(\ili{Geser} has functioned as a lingua franca in the region for many
				...
				which would regularly account for the \ili{Irarutu} word.
				Furthermore, \ili{Yamdena} has \it{bu{\tl}bus-e} ‘foam’.
				Thus, even if one rejects the doublet *busa as spurious (see
				footnote 15 and the discussion following \trf{11-24}),
				*j > \it{s} in the \ili{Irarutu} word would be paralleled by \ili{Yamdena} -- a
	...
	\item	*qaləjaw > \ili{Irarutu} \it{re} [re],
				...
				**lə with loss of the final syllable, including *j.
				\ili{Kuri} has \it{ror} [ror] ‘day’
				and \it{rror} [rəˈrɔːr] ‘daylight’ which preserves final *j > \it{r}.
				*qaləjaw > \ili{Kuri} \it{ror} requires *ə > \it{o},
				which is attested in other words.\footnote{
						Six reflexes of *ə
						have been identified in \ili{Kuri},
						of which two have *ə > \it{o}: *təlu > \it{tor} ‘three’ (\ili{Irarutu}
						\it{tor}) and *dəŋəR > \it{ya-v-nogr} ‘hear’ (\ili{Irarutu} \it{-f-nogr} ‘hear’).
						Other reflexes of *ə in \ili{Kuri} are: *qatəluR > \it{tru} ‘egg’ (\ili{Irarutu}
						\it{tru} ‘egg’), *taŋila > \it{tgra} [tə̆gra] ‘ear’ (\ili{Irarutu} \it{tgra} [təŋgəra]),
						*əsa > \it{eso} ‘one’ (\ili{Irarutu} \it{esu}),
						and *pusəj > \it{wuer} [wuɛrɛ] ‘navel’ (\ili{Irarutu} \it{fwir} ‘navel’).}
	\item	*qapəju > \ili{Irarutu} \it{-fir-} ‘bile’ is mostly unproblematic,
	...
	\item	*pusəj > \it{fwir} ‘navel,
				...
				which is not the normal reflex in \ili{Irarutu} but is attested in some words.
				...
				\ili{Kuri} has ‹ʷuèrè› [wuɛrɛ] ‘navel’ (probably phonemically /wuer/
				or /uer/ with final vowel epenthesis) which requires *p > \it{w} /{\gap}\it{u}.
				This is not implausible given \ili{Kuri} *p/*b > [b] {\tl} [β] {\tl}
				[f] {\tl} [v] elsewhere.
	\item	\it{wegur} [wɛŋgurə] ‘nose’ is potentially a reflex of
				...
				\ili{Kuri} has ‹ya-wegě› [yawegə̆] \it{ya-we-g} \tsc{1sg}-nose-\tsc{1sg}
				and ‹iwè› \it{i-we} \tsc{3sg}-nose \citep[34]{SmitsVoorhoeve1992a},
				thus providing comparative support for analysing the initial
				two segments of \ili{Irarutu} \it{wegur} as a historically independent morpheme.
...
then placing \ili{Irarutu} within this node would rest on only two
...
‘foam’ which is also be found in \ili{Yamdena} \it{bu{\tl}bus-e}.

On the other hand,
there are four potential instances of *j > \it{r} though,
admittedly, all involve problems except *qaləjaw > \ili{Kuri} \it{ror} ‘day’.
Nonetheless, the explanations for the development of *qapəju
> \it{-fir-} ‘bile’ and *pusəj > \ili{Irarutu} \it{fwir},
\ili{Kuri} \it{(w)uer} ‘navel’ find parallels in other sound changes.
At the very worst,
...
evidence for including \ili{Irarutu} in SHWNG.

On the other hand,
there \it{is} positive evidence from the sound changes for including
\ili{Irarutu} and \ili{Kuri} in \tsc{Tanimbar-Bomberai}.
It shares the merger of *z/*d > **d,
as attested by *zalan ‘road’ > \ili{Fruata} \ili{Irarutu} \it{radni},
\ili{Kaimana} \ili{Irarutu} \it{rarn}, \ili{Kuri} \it{rarn}
and perhaps also *zauq ‘far’ > \ili{Irarutu} \it{nero},
\ili{Kuri} \it{gero} (the second syllables).
In addition, \ili{Irarutu} and \ili{Kuri} share *j > \it{r} in four of the
six words for which *j > **r posited for \tsc{Tanimbar-Bomberai} (\trf{11-25}).

Within \tsc{Tanimbar-Bomberai}, \ili{Irarutu} and \ili{Kuri} share *l > \it{r},
