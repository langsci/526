...
different from \tsc{Sula-Buru}, and in particular from \ili{Buru} to a degree that is far different
...
and \tsc{Sula-Buru} languages to the south\ili{east},
...
\ili{Buru} has the expected \it{hoso-n} ‘ribcage’,
whereas \ili{Soboyo} has unexpected \it{usuk} ‘rib’.
...
\ili{Buru}-\ili{Lisela} only have  *R > \it{h}.
\ili{Sanana}-\ili{Mangole} have *R, *s > \it{h}.
\ili{Sanana} also has *d > \it{h}.
\ili{Ambelau} has *R, *l, *d > \it{h}.
\citet{Blust1981} observes that \ili{Soboyo} \it{h} is from multiple
sources, such as PMP *h,
*p, *R, *d, *r, *b.
\ili{Buru} consistently preserves the distinctions of those proto phonemes.

\tsc{\ili{Saluan}-Banggai} merges *j/*y > **y.
...
with \ili{Buru}-\ili{Lisela} **y > {\0} as a secondary development.
...
\tsc{\ili{Saluan}-Banggai}, and distinguish it from \tsc{Sula-Buru}.
...
than \tsc{Sula-Buru} and should not be included with other languages of ‘\ili{Central} Maluku’.
...
While \ili{Soboyo} might show some cases of PMP *h = \it{h} \citep{Blust1981},
the normal reflex for \ili{Soboyo}
...
noting that it shares some diagnostic sound changes with \tsc{\ili{Saluan}-Banggai},
but with the caveat that more data
and more research are clearly required.

The presence of the preposed possessor in \ili{Banggai} associated
with OV typology (not found in other \tsc{\ili{Saluan}-Banggai} languages)
...
and \tsc{\ili{Ambon}-Seram} languages, requires that some higher level
...
between at l\ili{east} some of the languages of Sulawesi
...
do show that there was at l\ili{east} one incursion into Maluku from
...
\tsc{Sula-Buru} subgroup, particularly in \ili{Mangole} and \ili{Sanana}, but also reaching \ili{Lisela},
\ili{Buru} and \ili{Ambelau} to lesser degrees.
