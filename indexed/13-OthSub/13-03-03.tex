...
it was shown that such markers are found in \tsc{\ili{Ambon}-Seram}
...
In the \tsc{\ili{Bima}-Lembata} language of \ili{Dhao} (\crf{ch:BL}),
...
the complex subject prefixes in \ili{Muna} \citep{vandenBerg1989},
and \ili{Busoa} \citep[54ff]{vandenBerg2020}.
...
\item	Their distribution is sporadic.
	\begin{itemize}
		...
		\item	\ili{As} noted in \srf{01-08},
					...
					So, for example, \tsc{\ili{Ambon}-Seram} forms
...
\item	The presence of subject markers does not define any of
			our higher level subgroups (except perhaps \tsc{\ili{Ambon}-Seram}),
...
\item	The forms that are cognate trace back to PMP
			...
			\ili{As} such they are not useful for linguistic subgrouping.
\end{itemize}
...
on verbs in Undergoer-prominent \tsc{\ili{Alor}-Pantar} languages which are typologically SOV.
\ili{Wersing} is an \tsc{\ili{Alor}-Pantar} language that has Undergoer prefixes
...
\begin{adjustbox}{width=\textwidth}
	...
	\begin{tabular}{ll|l|l|l|l@{ }l|l|ll|l}\lsptoprule
								&	\ili{Funai} 			&	\ili{Tii}		&	\ili{Tetun}	&	\ili{Idate}	&	\mc{2}{l|}{Tugun}				&	\ili{Luang}	&	\mc{2}{l|}{SE \ili{Babar}\su{Δ}}&	\ili{Selaru}\\
								&	\ili{Helong}\su{‡}&				&				&				&	\tsc{realis}&	\tsc{irr}	&				&						&								&	\\	\midrule
...
\tsc{3pl}				&	na-					&	ra-		&	r-/n-	&	ra-		&	ra-					&	mar-			&	ra-		&	te-				&	t-						&	ra-\\
	...
	\end{tabular}
		\begin{tablenotes}\tnsize
			...
			\item[‡]	{\ili{Funai} \ili{Helong} subject prefixes are identical to Roi{\Q}is \ili{Amarasi},
								from which they may be borrowed.
								\ili{Semau} \ili{Helong} has lost productive prefixes,
								but retains fossilised \it{na-} on some verbs.}
			\item[Δ]	{SE \ili{Babar} has undergone *n > \it{l},
								and *t > \it{k, x} thus partly explaining
								the \tsc{3sg} and \tsc{1pl.incl} prefixes.}
			\item[‖]	{\ili{Selaru} \it{i-} is used for animate subjects
...
\ili{As} can be seen from the diversity of forms in \trf{13-21},
it would be difficult to reconstruct subject prefixes to Proto-Timor-\ili{Babar},
...
and Undergoer suffixes/enclitics, such as in \tsc{\ili{Ambon}-Seram} languages (see \srf{07-04-03}).
This is reflected clearly in Split-S languages,
for example, in \tsc{\ili{Ambon}-Seram}
