...
as forming the basis for a “South\ili{east} Maluku” subgroup,
...
lll}
\tbx{13-13} & \mc{3}{l}{\ili{Fordata} subject prefixes} \\
						& \it{dava}		&						& `seek'\\
...
>{\hspace{-\tabcolsep}\enspace}
ll}
\tbx{13-14} & \mc{2}{l}{\ili{Yamdena} compounding} \\
						& \it{bali}		& `side'\\
						&	\it{duwe}		&	`two'	\\
...
>{\hspace{-\tabcolsep}\enspace}
lll}
\tbx{13-15} & \mc{3}{l}{\ili{Leti} and \ili{Moa} compounding \citep[172]{Jonker1932}} \\
						&				& \it{pipi}			& `goat'\\
						&				&	\it{duma}			&	< \ili{Malay} \it{domba} `sheep'	\\
						&	\ili{Leti}	&	\it{pipi-duma}	&	`sheep'	\\
						&	\ili{Moa}		&	\it{pipdiuma}		&	`sheep'	\\
\end{tabular}\mbox{}\\}
...
>{\hspace{-\tabcolsep}\enspace}
lll@{ }l@{ }l@{ }l@{ }l}
\tbx{13-16} & \mc{7}{l}{\ili{Yamdena} suffixing} \\
						&	\it{manik}		& `bird'	&&&&& \\
...
>{\hspace{-\tabcolsep}\enspace}
lll}
\tbx{13-17} & \mc{3}{l}{\ili{Selaru} suffixing} \\
						& \it{hahy}		&	`pig'			& \\
...
the morpho-phonology requires that \ili{Selaru} distinguish consonants
(C), vowels (V), and glides (G) as distinct emic categories in the phonotactics of \ili{Selaru}.
The data and analysis in example \qf{13-18} were provided by
David Coward (pers. comm. 21 Sept.
2020), illustrating a fuller paradigm for \ili{Selaru}.
For body parts, the clearest context to see whether a \ili{Selaru}
...
for \ili{Larike} and Tables \ref{tab:07-17} and \ref{tab:12-01} for \ili{Banda})
for examples of such morphophenemic alternations).
\citet[252f]{Mills1991} notes that binding has also been observed
in \ili{Biak}, \ili{Patani}, Misool (SHWNG), and West Sumba, and adds “\ili{As} 
...
\begin{quote}
		...
		Proto Tanimbar-\ili{Kei}, Numfor-\ili{Patani} (and perhaps Proto Timor
		...
		SHWNG group without Numfor/\ili{Biak}
		and \ili{Patani}, though I suspect not.
		...
		(a) PTK [Proto-Tanimbar-\ili{Kei}]/PLet [Proto-Lettic]/WSumba [West
		...
		East-\ili{Central} MP at l\ili{east})
...
for justifying “South\ili{east} Maluku” as a subgroup saying,
...
It thus seems premature to accept the existence of Mills’s ‘south\ili{east}
Maluku’ subgroup.” 
While we agree with Blust regarding the value of binding for
subgrouping “South\ili{east} Maluku”,
...
of morphophonemic alternations in subject prefixing in \tsc{\ili{Ambon}-Seram}
languages, \ili{Banda}, and \ili{Geser} (the latter a \tsc{Seram-Tanimbar-Bomberai} language).
...
and palatalization found in \ili{Yamdena},
\ili{Selaru}, \ili{Luang}, \ili{Leti}, etc.).
...
The morphophonemic alternations in \tsc{\ili{Ambon}-Seram},
\ili{Banda} and \tsc{Seram-Tanimbar-Bomberai} languages are described in \srf{07-04-03}.
