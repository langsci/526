...
\tsc{\ili{Ambon}-Seram} is given values of \it{p-,
-f-, -f}, as the values for those positions,
even though most \tsc{\ili{Ambon}-Seram} languages in most nodes
...
\ili{Banda}			&	{\hsh}{\0}&	{\hd}{\0}	&	{\hr}p				&	{\hsh}f		&	{\hd}f		&	{\hr}mb		&	{\hr}mb		&	{\hd}mb		&	{\hd}mb\\
...
but they appear to remain distinct in \tsc{\ili{Bima}-Lembata}.
Note also that in \tsc{\ili{Ambon}-Seram} the merger is devoiced to
...
such as *upu > \ili{Buru} \it{opo} ‘grandparent, grandchild (reciprocal term)’.
In \ili{Buru} *p = \it{p}
and *mp > \it{b} are regular.
*umpu would derive an unattested *[obo]; compare that with PMP *ta-umpu ‘grandparent/grandchild’ (cf.
Dempwolff gloss ‘master, lord’) > \ili{Buru} \it{tobo-n} ‘master, lord’.
...
the optional nasal, and exceptions like \ili{Buru} diminish its reliability
...
\begin{enumerate}
	\item	Retain the labial with no voicing,
				...
				as in *ma-panas ‘hot’ > \it{hanas} ‘hot’ (\ili{Tetun}).
	...
	\item	Retain the nasal of the prefix
				...
				as in *ma-panas ‘hot’ > \ili{Meto} \it{manas} (‘sun’ in most \ili{Meto} languages;
				‘hot’ in \ili{Kusa-Manea}); *ma-putiq ‘white’ > \ili{Meto} \it{mutiʔ} ‘white, silver’.
	\item	*ma-p > \it{p},
				but not merge with *p,
				and no post-nasal voicing,
				as in \ili{Dhao} *ma-panas > \it{pana} ‘hot’,
				*ma-putiq > \it{puɖ͡ʐi} ‘white,
				silver’, *ma-pənuq > \it{pənu} ‘full’; (\ili{Dhao} *p > {\0},
				\it{p} as in *pajay > \it{are} ‘rice’,
				*pusuq > \it{usu} ‘heart’,
				*pitu > \it{piɖ͡ʐu} ‘seven’).
				Or \ili{Alune} *ma-putiq > \it{puti} ‘white’,
				*ma-pənuq > \it{penu} ‘full’; (\ili{Alune} *p > {\0} as in *pajay >
				\it{ala} ‘rice’, *pija > \it{ila} ‘how many’,
				*punti > \it{uri} ‘banana’,
				*hapuy > \it{au-e} ‘fire’; note that \ili{Alune} retains voicing in
				*b = \it{b}; see \srf{10-02}).
	\item	*ma-p > \it{b} becoming voiced (= “post-nasal voicing of
				stops” as described by Blust),
				as in \ili{Buru} *ma-panas ‘hot’ > \it{bana} ‘fire’,
	...
	\item	Retain the nasal,
				...
				become voiced as in \ili{Kambera} *ma-panas > \it{mbana},
				‘hot, warm’, \it{mbanah-u} ‘hot’.
	\item	Retain both the full historical prefix with the vowel
				and the labial, and stay voiceless as in *ma-panas ‘hot’ > \ili{Gorom}
				\it{mahanas} ‘hot’, \ili{Geser} \it{mfanas} ‘hot’, \ili{Yamdena} \it{mefanas} ‘hot’; *ma-putiq
				‘white’ > \ili{Gorom} \it{mahuti} ‘white’,
				\ili{Geser} \it{mafuti} ‘white’, \ili{Yamdena} \it{mafuti} ‘white’.
\end{enumerate}
...
However *ma-b = \it{mab} in *ma-bəRat ‘heavy’ > \ili{Yamdena} \it{maberat}
...
*mp/*mb/*ma-p/*ma-b > **mb for both \tsc{Sula-Buru} and \ili{Banda}.\footnote{
		In \tsc{Sula-Buru} languages, the surface manifestation of **mb > \it{b},
		whereas \ili{Banda} retains the nasal-consonant cluster **mb = \it{mb}.}
...
Such complexity is shown for a number of closely related \ili{Meto}
...
%\endhead
	&	\ili{Meto} (most documented varieties)	&	\tbr{m}anas	&	sun	\\
	&	\ili{Kusa-Manea}	&	\tbr{m}anas	&	hot	\\
	&	\ili{Timaus}	&	na-\tbr{m}ana	&	it is midday	\\
	&	\ili{Meto} (most documented varieties)	&	na-\tbr{h}ana	&	cook (< *panas)	\\
	&	\ili{Kotos} \ili{Amarasi}	&	k-\tbr{h}anas	&	drought, hot/dry season (*ka-panas)	\\
\lspbottomrule
\end{tabularx}\mbox{}\\}

\tsc{Southwest Maluku} languages of the \tsc{\ili{Kisar}-Luang} node of \tsc{Timor-Babar}
...
%\endhead
	&	\ili{Kisar}	&	\tbr{m}anha	&	hot	\\
	&	\ili{Roma}	&	\tbr{m}ahan	&	hot	\\
	&	\ili{Luang}	&	\tbr{p}anha	&	hot (**mpanas; cf. *umpu > \it{upə} grandchild)	\\
	&	\ili{Wetan}	&	\tbr{p}aana	&	hot (**mpanas)	\\	\midrule
\tbx{13-03}	&	PMP	&	*\tbr{ma}-\tbr{p}ənuq	&	full	\\
	&	\ili{Kisar}	&	\tbr{p}enu	&	full	\\
	&	\ili{Roma}	&	\tbr{p}empwenu	&	full (/penu{\tl}penu/)	\\
	&	\ili{Luang}	&	\tbr{p}en-penu	&	full	\\
	&	\ili{Wetan}	&	\tbr{p}eni	&	full	\\
\lspbottomrule
\end{tabular}\mbox{}\\}
...
Superficially it appears that \tsc{\ili{Ambon}-Seram},
\ili{Banda}, \tsc{Aru}, \tsc{Seram-Tanimbar-Bomberai}, and \tsc{Timor-Babar}
...
For example, \tsc{\ili{Ambon}-Seram} retains initial *p = \it{{\#}p} in
...
\ili{Tehoru} \it{puse-n} ‘navel’, \ili{Alune} \it{benu} ‘sea turtle’,
and reflexes of causative *pa- = \it{pa-} in several languages).
\ili{Banda} retains final *p in *qatəp > \it{atop} ‘thatch’.
In \tsc{Seram-Tanimbar-Bomberai}, \ili{Kur} retains *p = \it{p} in all positions,
and \ili{Yamdena} retains final *p = \it{p{\#}} (e.g.
...
The exceptions word initially in some \tsc{\ili{Ambon}-Seram} languages,
in all positions in \ili{Kur},
and word finally in \ili{Yamdena}
...
modified “\ili{Central} Malayo-Polynesian” subgroup,
although how it links up to PMP,
and how it relates to \tsc{\ili{Bima}-Lembata},
\tsc{\ili{Central} Timor}, \tsc{Sula-Buru}, SHWNG and Oceanic remains elusive.
