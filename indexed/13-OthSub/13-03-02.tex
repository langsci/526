...
\begin{table}\tcp{Comparing \ili{Havu} and \ili{Dhao} pronominal systems}{13-14}
%\begin{adjustbox}{width=\textwidth}
	\begin{threeparttable}\st{6pt}
	\begin{tabular}{lllll|ll}\lsptoprule
								&	\mc{4}{l|}{Dhao}															&	\ili{Havu}													\\
...
\lspbottomrule
	\end{tabular}
		\begin{tablenotes}\tnsize
			...
			\item[Δ]	{The dominant \ili{Seba} dialect uses \tsc{1sg} \it{yaa}.
								\ili{Dimu} and \ili{Raijua} Wawa use \it{ʄaa}.
								\ili{Raijua} D{\Q}ida uses \it{ʤoo}. \tsc{3pl} \it{noo} is \ili{Raijua}.}
		\end{tablenotes}
	\end{threeparttable}
%\end{adjustbox}
\end{table}

Closely related \tsc{\ili{Bima}-Lembata} languages \ili{Havu}
and \ili{Dhao} use their free pronouns as subject,
...
\ili{Dhao} clitics can be used as proclitics or more commonly as enclitics.
...
Many \ili{Dhao} verbs are not inflected for subject.
...
The pronominal systems of several \tsc{\ili{Ambon}-Seram} languages
and \ili{Banda} were illustrated in \crf{ch:CM},
...
\tsc{Sula-Buru} languages are also very different to the \tsc{\ili{Havu}-Dhao}
forms, functions, and distribution in \trf{13-14}.

\begin{table}\tcp{\ili{Buru} pronominal sets}{13-15}
\begin{adjustbox}{width=\textwidth}
...
\lspbottomrule
	\end{tabular}
		\begin{tablenotes}\tnsize
			\item[†]	{In the \ili{Masarete} dialect the genitive system has collapsed to
								...
								system for \ili{Laha}, spoken around the airport on \ili{Ambon}.}
			\item[‡]	{The \ili{Masarete} dialect of \ili{Buru} uses \it{nunu}.
								\ili{Rana} and Wae Sama use \it{nini}.
								Related \ili{Lisela} also uses \it{nini}.}
		\end{tablenotes}
	\end{threeparttable}
\end{adjustbox}
\end{table}

The \ili{Buru} data in \trf{13-15} are adapted from \citet{Grimes1991c}.
...
It is still productive in the \ili{Rana} dialect,
but obsolete in the \ili{Masarete} dialect.

The \tsc{Seram-Tanimbar-Bomberai} language \ili{Kowiai} similarly neutralises
...
\begin{table}
	\centering\tcp{\ili{Kowiai} prominal sets}{13-16}
...
as \ili{Helong} (\srf{04-01}, see also \citealp[75]{Edwards2020}),
\tsc{Eastern Timor} languages such as \ili{Tetun} (\srf{04-03}),
\tsc{\ili{Bima}-Lembata} languages such as \ili{Havu}
and \ili{Dhao} (\srf{03-03}, see also \citealp[264]{Grimes2010a}),
or \tsc{\ili{Central} Timor} languages such as \ili{Kemak} (\crf{ch:CT}).
...
Pronouns from \ili{Meto} languages are illustrated in \trf{13-17} by
data from \ili{Kotos} \ili{Amarasi}
(adapted from \citealp{GrimesBaniCaet2012} and \citealp[262]{Edwards2020}).

\begin{table}
	\centering\tcp{\tsc{Meto} (\ili{Kotos} \ili{Amarasi}) pronominals}{13-17}
...
A and O \citep{Dixon1994}, \ili{Amarasi} S
...
\ili{As} mentioned above, oblique arguments in \ili{Amarasi} use the accusative
...
Pronominals from \ili{Dela} (\tsc{West Rote-Meto}) are illustrated in \trf{13-18}.
\ili{Dela} does not have a full paradigm of nominative
...
\ili{Dela} also has vocative pronouns for \tsc{1pl.incl} and \tsc{2pl}.
The accusative pronouns also function as the object of prepositional
verbs such as \it{noo} ‘with’,
\it{neu} ‘to/for’, or \it{mia} ‘from’.

\begin{table}
	\centering\tcp{\ili{Dela} pronominals \citep[97]{Tamelan2021}}{13-18}
...
In the \tsc{Nuclear Rote} language \ili{Rikou},
spoken in eastern Rote,
the distinction between \tsc{nom}
and \tsc{acc} is found only in third person pronouns.
\ili{Rikou} pronominals are given in \trf{13-19}.
\ili{Rikou} data are gleaned from \citet{UBB2020b}
and \citet{Jonker1908,Jonker1915}.

\begin{table}
	\centering\tcp{\ili{Rikou} pronominals}{13-19}
...
There is variation in \ili{Rikou} regarding which  form the object
...
\begin{table}
	\centering\tcp{\ili{Rikou} prepositional verbs}{13-20}
	\st{6pt}\begin{tabular}{llllll}\lsptoprule
...
\ili{As} glimpsed in the preceding discussion,
