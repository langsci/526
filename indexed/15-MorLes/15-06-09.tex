...
\tsc{\ili{Ambon}-Seram}, \tsc{Aru}, and \tsc{Timor-Babar}.
...
\ili{Watubela} (\tsc{Eastern Islands}) retains PMP *q in all positions.
\ili{Yamdena} (\tsc{Tanimbar-Bomberai}) retains a number of conservative
features, such as *b = \it{b},
*d = \it{d /{\#}{\gap}},
*p = \it{p}{\#} and retains many trisyllables
and final consonants, among other things.
\ili{Kur} (\tsc{\ili{Teor}-Kur}) retains *p = \it{p} in all positions.
Several STB languages also maintain a distinction between *ma-p and *ma-b.

It is not yet clear where the homeland is for STB.
Given the conservative features of \ili{Yamdena},
...
contains languages in both the Tanimbar Islands (\ili{Fordata})
...
and SHWNG that need to be explored further.\footnote{
		...
		Bomberai Peninsula: \ili{Arguni}, \ili{Bedoanas} \it{natu}, \ili{Erokwanas} \it{natuʔ} ‘small’,
		\ili{Irarutu} \it{ntü}
		‘smallest member of a set/offspring,
		small young’, as well as \ili{Kowiai} \it{net} ‘small’.}
In \crf{ch:STB}
(\srf{11-01}) we noted that STB is somewhat weakly defined
and it may be better to identify its constituent nodes (\tsc{East
Seram}, \tsc{Eastern Islands}, \tsc{\ili{Teor}-Kur}, \tsc{Tanimbar-Bomberai})
as independent subgroups.
Given this, and the similarities between STB
and SHWNG, it could be that (parts of?) STB were
settled from “north\ili{east} Indonesia” (\srf{15-06-03}).

But, like the problem above with \tsc{Sula-Buru}
and \tsc{\ili{Ambon}-Seram}, it is difficult to line up the discontinuities
in the sound correspondences to see STB deriving other subgroups
such as \tsc{Sula-Buru}, \tsc{\ili{Ambon}-Seram}, \tsc{Timor-Babar}, or \tsc{\ili{Bima}-Lembata}.
Or vice versa.
