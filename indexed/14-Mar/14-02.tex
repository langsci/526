...
phonology for at l\ili{east} one incursion from Sulawesi eastward into
Maluku, from central Sulawesi through \ili{Banggai} to \ili{Taliabu}.\footnote{
		The distribution of the \it{Strigocuscus pelengensis} ‘\ili{Banggai}
		(dwarf) cuscus’, found in the \ili{Banggai} Islands
		and the nearby Sula Islands,
		parallels the linguistic link between \tsc{Taliabu}
		and \ili{Banggai} discussed in \crf{ch:SB}.
		The distance between \ili{Sanana}
		and \ili{Buru} appears to have been too great for that species to cross.}
...
in Dutch sources of that era with \ili{Malay} terms for ‘cuscus’,
...
In Blust's view, the breakup of PCEMP happened somewhere in “north\ili{east} Indonesia”.
The group that eventually became \tsc{POc} travelled eastward
and dispersed from there throughout Oceania.
\citet[98]{Blust2008} says, “\ili{As} noted in \citet{Blust1993},
\ili{Central} Malayo-Polynesian almost certainly was the product of
rapid movement from north\ili{east} Indonesia through the central
...
arisen.” 
Given that back-eddy type of scenario dispersing from ``north\ili{east}
...
\ili{As} yet there is little or no linguistic or archaeological evidence
to support Blust’s purported migration scenario from north\ili{east}
...
\begin{enumerate}
	...
	\item	Languages reflecting **kVndoR(a) are extremely limited
				...
				to the west, and to Papuan languages in the north and \ili{east}.
...
\ili{As} mentioned above, we note that reflexes of the two types of
...
\begin{enumerate}
	...
	\item	Words related to **kusay in Sulawesi refer to ‘bear cuscus’.
				...
				**taŋali, for the ‘dwarf cuscus’.
				Words related to **mantəR in \tsc{\ili{Muna}-Buton} (\tsc{Celebic}) languages
	...
	\item	The few languages that may appear to have both the \it{k}-initial
				...
				For example, most varieties of \ili{Cia-Cia} in South\ili{east} Sulawesi
				have only \it{mante} ‘bear cuscus’ (from **mantəR),
				but the \ili{Wasambua} dialect of \ili{Cia-Cia} has only \it{kuse} ‘bear cuscus’ (from **kusay).
\end{enumerate}