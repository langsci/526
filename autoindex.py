...
def process_file(filename):
    ...
    # print stats
    print(
        " %s now contains %i indexed languages and %i indexed subject terms"
        % (filename.split("/")[-1], numberoflanguages, numberofterms)
    )
...
if __name__ == "__main__":
    ...
    for lg in lgs:
        if "(" in lg:
            print(
                f"{lg} contains parentheses. Parentheses are not allowed in indexed language names. Exiting."
            )
            sys.exit()
    terms = open("localsubjectterms.txt").read().split("\n")
    for term in reversed(
        sorted(terms, key=len)
    ):  # start with longest term to avoid double indexing
        if "(" in term:
            print(
                f"{term} contains parentheses. Parentheses are not allowed in indexed language names. Exiting."
            )
            sys.exit()
